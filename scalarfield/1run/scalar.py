...
class scalarpow(object):
	...
	"""
	def __init__(self,gamma_i,Ophi_i,li,n,h=0.7,Ob0=0.045,ns=0.96,sigma_8 = 0.8):
	...
	def sol(self):
		inic = [self.gamma_i,self.Ophi_i,self.li]
		res = odeint(self.f, inic, self.n1,mxstep=1000)
		return res
	...
	def ophi_z(self,z):
        ...
        def phi_pres(self):
                """
                present day value of the field (Phi_0)
                """
                phi0 = -self.n / self.sol()[999,2]
                return phi0


        def phi(self,N):
                """
                Field as a function of log(a)
                """
                phi1 = -self.n / self.sol()[:,2]
                phi2=UnivariateSpline(self.n1,phi1,k=3,s=0)
                return phi2(N)

        def phi_z(self,z):
                """
                Field as a function of redshift z
                """
                return self.phi(np.log(1./(1.+z)))
	...
	def time_delay_dis(self,zd,zs):
        ...
        def alpha_integrand(self,z):
                  if  self.eqn_state_z(z) >= -1.:
                         result = np.sqrt(3. * self.ophi_z(z) *(1.+self.eqn_state_z(z)))/(1.+z)
                  elif (self.eqn_state_z(z) < -1.):
                         #Phantom field, but similar implications 
                         result = -np.sqrt(3. * self.ophi_z(z) *np.abs(1.+self.eqn_state_z(z)))/(1.+z)
                  return result

        def alpha_var(self,z,zeta):
               """ The alpha variation from z0 to z.
    
               Dimensionless

               """
               I = quad(self.alpha_integrand, 0., z,limit=1000,epsabs=1e-10)
               return (zeta * I[0])
...
class scalarexp(scalarpow):
	...
	def __init__(self,gamma_i,Ophi_i,li,h=0.7,Ob0=0.045,ns=0.96,sigma_8 = 0.8):
                self.gamma_i = float(gamma_i)
		self.Ophi_i = float(Ophi_i)
		self.li = float(li)
		self.Ob0 = float(Ob0)
